...
-- for pystr::is_space in particular.

theorem strip_removes_a_prefix_and_a_suffix_of_space (p : Char → Bool) (s : List Char) :
    ∃ pre suf, s = pre ++ strip p s ++ suf ∧
      (∀ c ∈ pre, p c = true) ∧ (∀ c ∈ suf, p c = true) := by
  obtain ⟨pre, hpre, hprep⟩ := lstrip_decomp p s
  obtain ⟨suf, hsuf, hsufp⟩ := rstrip_decomp p (lstrip p s)
  refine ⟨pre, suf, ?_, hprep, hsufp⟩
  have hjoin : strip p s ++ suf = lstrip p s := by
    unfold strip
    exact hsuf.symm
  rw [List.append_assoc, hjoin]
  exact hpre

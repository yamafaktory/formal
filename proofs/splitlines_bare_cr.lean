...
-- boundaries; the boundary set is the ten characters the Rust match names. The
-- hypothesis on b says only that the carriage return is not followed by a line
-- feed, which includes the case where it ends the text.

theorem a_bare_cr_is_a_boundary (a : List Char) (h : ∀ c ∈ a, isBreak c = false)
    (b : List Char) (hb : ∀ t, b ≠ '\n' :: t) :
    splitlines (a ++ '\r' :: b) = a :: splitlines b := by
  unfold splitlines
  rw [go_prefix a h ('\r' :: b) []]
  cases b with
  | nil => simp [go, isBreak]
  | cons d rest =>
    have hd : d ≠ '\n' := by
      intro hh
      exact hb rest (by rw [hh])
    simp [go, isBreak, hd]

...
theorem strip_idempotent (p : Char → Bool) (s : List Char) :
    strip p (strip p s) = strip p s := by
  have h1 : lstrip p (strip p s) = strip p s := by
    cases hs : strip p s with
    | nil => simp [lstrip]
    | cons c t => simp [lstrip, strip_head p s c t hs]
  unfold strip at h1 ⊢
  rw [h1]
  exact rstrip_idem p (lstrip p s)

...
-- boundaries; the boundary set is the ten characters the Rust match names, with
-- '\r' handled by the bare-carriage-return property instead.

theorem a_break_ends_the_line_before_it (a : List Char) (h : ∀ c ∈ a, isBreak c = false)
    (br : Char) (hbr : isBreak br = true) (hcr : br ≠ '\r') (b : List Char) :
    splitlines (a ++ br :: b) = a :: splitlines b := by
  unfold splitlines
  rw [go_prefix a h (br :: b) []]
  cases b with
  | nil => simp [go, hbr]
  | cons d rest => simp [go, hbr, hcr]

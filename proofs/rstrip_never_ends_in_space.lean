...
-- Assumptions: text modelled as List Char; trim_end_matches modelled as dropping a
-- maximal run from the end; quantified over the trimming predicate p, so it holds
-- for pystr::is_space in particular.

theorem rstrip_never_ends_in_space (p : Char → Bool) (s t : List Char) (c : Char)
    (h : rstrip p s = t ++ [c]) : p c = false :=
  rstrip_last p s t c h

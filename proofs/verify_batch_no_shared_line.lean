...
theorem a_later_entry_starts_after_this_one_ends
    (ms : List Nat) (k m : Nat) (s : Nat × Nat) (hs : s ∈ spans ms (k + m + 3)) :
    k + 1 + m < s.1 := by
  have := every_span_starts_after_where_it_was_laid_out ms (k + m + 3) s hs
  omega

-- The guarantee verify_batch rests on: it hands an error to whichever entry covers
-- its line, so no line may be covered by two entries or one error would be reported
-- against two different proofs.

theorem no_line_is_claimed_by_two_entries
    (ms : List Nat) (k m line : Nat) (s : Nat × Nat)
    (hs : s ∈ spans ms (k + m + 3)) (hhere : covers (k + 2, k + 1 + m) line) :
    ¬ covers s line := by
  have hlater := a_later_entry_starts_after_this_one_ends ms k m s hs
  simp only [covers] at hhere ⊢
  omega

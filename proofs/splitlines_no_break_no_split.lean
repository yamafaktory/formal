...
theorem text_without_a_break_is_one_line (s : List Char) (h : ∀ c ∈ s, isBreak c = false) :
    splitlines s = if s = [] then [] else [s] := by
  have key := go_prefix s h [] []
  simp only [List.append_nil, List.nil_append] at key
  unfold splitlines
  rw [key]
  by_cases hs : s = [] <;> simp [go, hs]
